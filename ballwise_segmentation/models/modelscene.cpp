...
//	cout<<"Canonical length = "<<float(category_length)/category_nums<<endl;
	...
	while(!feof(fp))
	{
		for(int m=0;m<5;m++)
			fscanf(fp, "%f ", &scores[pos][m]);
		fscanf(fp, "%f ", &scores[pos][6]);// swapping the player scores for 2nd Innings
		fscanf(fp, "%f ", &scores[pos][5]); 
		pos++;
	}
